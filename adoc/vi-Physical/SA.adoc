...
We have seen that resources are the most important elements in terraform, and there is an other resource type used as last step here, the _Provisioner_ resource.
They can be used to model specific actions on a remote machine in order to prepare them for other services.

The file provisioner is used to copy files or directories from the machine executing Terraform to the newly created resource and the remote-exec provisioner is usesd to a script on a remote resource after it is created run run the salt configuration management.
...
//fixme
which are triggered by the terraform module _MAIN/generic_modules/salt_provisioner/main.tf_ .
