...
. Go one directory up and change into the _pillar_example_ directory and then into the _automatic_ directory, where you can see 3 additional directories. They will provide the configuration variable for the relevant services. This automatic folder will work for all cloud providers we support today.
...
disks_size:: the size of the additional disk in GB, as every size has certain IOPS caps
caching:: the caching recommendations for Azure premium disks assume the I/O
