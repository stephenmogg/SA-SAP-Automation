...
. Copy the file _terraform.tfvars.example_ to _terraform.tfvars.example_
    ...
    In order to peform a simple deployment, only update the parameters as listed below.
...
+
SUSE recommend to create new sshkeys for the deployent as both public and private keys will need to be provided as they are copied to the cluster nodes as part of the deployment.
...
This setting provides Terraform the credentials to deploy infrastrucuture on the AWS cloud.
...
One of the key points to consider in an SAP deployment is sizing and applies across three key areas: compute power, storage space and i/o capacity and network bandwith.
...
The SUSE Auomation project uses custom grains to match the roles and the further states.
...
If targetting a directory during a _state.apply_ or in the state Top file, Salt looks for an init.sls file in that directory and applies it.
...
If using the formula standalone the data needs to be provided manually.
...
The _habootstrap-formula_ provides the needed cluster setup for SAP HANA, SAP Netweaver and if needed for the HA NFS service built with drbd.
