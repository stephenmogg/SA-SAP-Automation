...
Please have a deep look at all variable and the comments for it,
to get aware whats is possible.
...
Formulars: group of states give a context for building blocks e.g HANA
...
You will see withing this directory structure all needed steps depending on the _role_ of the node.

If we look deeper into one of the directories, e.g. _hana-node_ we will find more files
...
Within the os setup there is e.g. the minion configuration file
how and where the minion looks for salt states.
...
One interesting file is the _minion_configuration.sls_ as it provides the information how and where salt search for the sls files.
