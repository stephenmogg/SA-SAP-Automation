...
SUSE Linux Enterprise Server for SAP Application is the bundle product for SAP users based on SUSE Linux Enterprise Server and the High Availablity Extension with many helpers for running SAP Application.
...
Therefore the natural name for such modules is "provider". So for example the _Azure Provider_ can be used to configure infrastructure in Microsoft Azure using the Azure Resource Manager API's.
...
We provide Terraform configuration files for AWS, Azure, GCP and libvirt.
...
Salt Client (Minion) only. All steps must be executed in all of the Salt clients where the formulas are going to be executed. This is the approach utilized in this framework as we do not need a central master system.
