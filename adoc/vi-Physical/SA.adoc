...
If the setup is using HA for SAP Netweaver or for SAP HANA or with the NFS service and there is mechanism for fencing of the virtual machines over an API, we use the SUSE SBD-device method. Such a SBD-Device is normally a raw shared disk beween two nodes.
