...
// - @PS - What are we trying to say here? ----> , this can be seen when looking at the configuration details of infrastructure components, features and management or security or scalability, etc.
...
Analysis may also show that more storage throughput than initially suggested is required, in which case increasing the capacity, IOPS or throughput of the storage may be needed. 
...
In addition, the formula sets up all of the pre-requirements as:
